...
ENCODING = 'utf-8'
SERVER_ADDRESS = ('127.0.0.1', 15661)
...
def send_to(participant: Participant, message: str):
	print("Sending message to "+participant.username+" : "+message)
	if message==None:
...
def handle(participant: Participant):
	...
	while True:
		...
		#try:
		#participant.client_socket.close()
		#broadcast(f'-- {participant.username} left the chat')
		#finally:
		#break
...
def receive():
	...
	while True:
		...
		if username in list(map(lambda p: p.username, participants)): ### SIGN IN PART
			found=False
			for participant in participants:
				...
				if participant.username==username and participant.password!=password:
					...
					while True: 
						...
						if password2==participant.password:
							found=True
							break
			if found:
				participant.client_socket=client_socket
				print(username+" joined back")

				send_to(participant,'%CONNECT%')
				thread = threading.Thread(target=handle, args=(participant,))
				thread.start()
		else:### SIGN UP PART
			new_participant = Participant(username,password ,client_socket)
			participants.append(new_participant)
			print("New participant: "+username)
			send_to(new_participant, '-- Connected to server!')
			send_to(new_participant,'%CONNECT%')
